...
const wordmarkWidth = 200;
const glyphWidth = 50;
const logoHeight = 44;

interface GeometryOptions {
  style?: "wordmark" | "glyph";
  clearSpace?: number;
}

function geometry(opts: GeometryOptions = {}) {
...
interface LogoProps {
  geometryArgs?: GeometryOptions;
  glyph?: string;
  className?: string;
}

export default function Logo({ geometryArgs, glyph, className }: LogoProps) {
  const pathData = glyph === "mobile" ? glyphPath : `${textPath} ${glyphPath}`;
  const geo = geometry(geometryArgs);
