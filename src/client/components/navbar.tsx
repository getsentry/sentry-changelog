...
export function Navbar() {
  ...
  const handleShowActiveNavItem = useCallback((e: MouseEvent) => {
    if (
      dropdownRef.current &&
    ...
    <header className="bg-white sticky top-0 w-full z-50 py-4">
      <div className="lg:max-w-6xl md:max-w-3xl max-w-xl mx-auto px-8 sm:px-6 lg:px-8">
        <nav className="flex justify-between items-center text-primary flex-wrap">
          ...
          <ul
            className={`${
              isMenuVisible ? "flex" : "lg:flex hidden"
            } lg:items-center lg:flex-row flex-col basis-full lg:basis-0`}
          >
            {NAV_ITEMS.map(
              ({
                id,
                ...
                <li key={id} className="lg:ml-2 relative pt-4 lg:pt-0">
                  ...
                  >
                    ...
                    {(children?.length || withArrowBtn) && (
                      <Chevron
                        direction={activeNavItem === id ? "down" : "right"}
                      />
                    )}
                  </Button>
                        ...
                        <div className="flex gap-6 lg:gap-0 lg:flex-row flex-col ">
                          {children.map((navSubItem) => (
                            <div
                              key={navSubItem.id}
                              className={`${
                                navSubItem.title ? "p-4" : "lg:px-4"
                              } lg:w-[${(1 / children.length) * 100}%]`}
                            >
                              {navSubItem.title && (
                              ...
                              <ul
                                className={`${
                                  navSubItem.title
                                    ? "lg:gap-0 grid sm:grid-cols-1 md:grid-cols-2 lg:flex lg:flex-col"
                                    : "gap-2 flex flex-col"
                                }`}
                              >
